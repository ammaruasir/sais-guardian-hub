...
import { consultantStatusLabel, specializationLabel, type Consultant, type ConsultantStatus, type Specialization } from "@/data/consultants";
      ...
      <div className="space-y-5">
        ...
        <div className="rounded-lg border bg-card">
          <Table>
            ...
            <TableBody>
              {rows.length === 0 ? (
                <TableRow><TableCell colSpan={7} className="py-10 text-center text-sm text-muted-foreground">لا توجد نتائج مطابقة</TableCell></TableRow>
              ) : rows.map((c) => {
                  ...
                  <TableRow key={c.id}>
                    ...
                    <TableCell>
                      <div className="flex gap-1">
                        ...
                        <Button variant="ghost" size="icon" className="text-destructive hover:bg-destructive/10" onClick={() => setToDelete(c)}><Trash2 className="h-4 w-4" /></Button>
                      </div>
                    </TableCell>
                  </TableRow>
                );
        ...
        <ConfirmDialog
          ...
          onConfirm={() => {
            if (toDelete) {
              ...
              addAudit({ user: "Admin User", type: "حذف استشاري", description: `حذف ${toDelete.nameAr}`, page: "consultants/", level: "warning" });
