...
export function ConsultantFormDialog({
  ...
  const submit = () => {
    if (!form.nameAr.trim() || !form.licenseNo.trim()) {
      toast.error("الاسم ورقم الترخيص مطلوبان");
      return;
    }
    if (initial) {
      ...
      <DialogContent dir="rtl" className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="text-end">
            {initial ? "تعديل استشاري" : "إضافة استشاري"}
          </DialogTitle>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2">
            ...
            <Select
              value={form.status}
              onValueChange={(v) => setForm({ ...form, status: v as ConsultantStatus })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
