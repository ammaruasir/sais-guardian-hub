...
}: { open: boolean; onOpenChange: (v: boolean) => void; initial?: Company | null }) {
  const addCompany = useAppStore((s) => s.addCompany);
  const updateCompany = useAppStore((s) => s.updateCompany);
  const addAudit = useAppStore((s) => s.addAudit);

  const [form, setForm] = useState<Company>({
    id: "", nameAr: "", nameEn: "", sector: "industrial", facilitiesCount: 1, activeProjects: 0, compliance: "review",
  });
  useEffect(() => { if (initial) setForm(initial); else setForm({ id: "", nameAr: "", nameEn: "", sector: "industrial", facilitiesCount: 1, activeProjects: 0, compliance: "review" }); }, [initial, open]);

  const submit = () => {
    if (!form.nameAr.trim()) { toast.error("الاسم بالعربية مطلوب"); return; }
    if (initial) {
      updateCompany(initial.id, form);
      addAudit({ user: "Admin User", type: "تعديل منشأة", description: `تعديل ${form.nameAr}`, page: `companies/${initial.id}`, level: "info" });
      toast.success("تم تحديث المنشأة");
    } else {
      const id = `co${Date.now()}`;
      addCompany({ ...form, id });
      addAudit({ user: "Admin User", type: "إنشاء منشأة", description: `إضافة ${form.nameAr}`, page: `companies/${id}`, level: "info" });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent dir="rtl" className="sm:max-w-lg">
        <DialogHeader><DialogTitle className="text-right">{initial ? "تعديل منشأة" : "إضافة منشأة"}</DialogTitle></DialogHeader>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div><Label>عدد المنشآت</Label><Input type="number" value={form.facilitiesCount} onChange={(e) => setForm({ ...form, facilitiesCount: Number(e.target.value) })} /></div>
          <div className="col-span-2">
            <Label>حالة الامتثال</Label>
            <Select value={form.compliance} onValueChange={(v) => setForm({ ...form, compliance: v as Company["compliance"] })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
