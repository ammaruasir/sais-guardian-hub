...
export function ProjectsFilters({
  ...
  return (
    <div className="grid gap-2 rounded-xl border border-border bg-card p-3 md:grid-cols-3 lg:grid-cols-7">
      ...
      <Select value={value.sector} onValueChange={(v) => set("sector", v as Sector | "all")}>
        <SelectTrigger>
          <SelectValue placeholder="القطاع" />
        </SelectTrigger>
        <SelectContent>
          <SelectItem value="all">كل القطاعات</SelectItem>
          {Object.entries(sectorLabel).map(([k, v]) => (
      ...
      <Select
        ...
        onValueChange={(v) => set("stage", v === "all" ? "all" : (Number(v) as Stage))}
      >
        <SelectTrigger>
      ...
      <Select value={value.company} onValueChange={(v) => set("company", v)}>
        <SelectTrigger>
          <SelectValue placeholder="المنشأة" />
        </SelectTrigger>
        <SelectContent>
      ...
      <Select value={value.reviewer} onValueChange={(v) => set("reviewer", v)}>
        <SelectTrigger>
          <SelectValue placeholder="المراجع" />
        </SelectTrigger>
        <SelectContent>
      ...
      <Button
        ...
        className="gap-1 lg:col-span-7 lg:justify-self-start"
      >
