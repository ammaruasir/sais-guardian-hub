  ...
  const submit = () => {
    ...
    if (initial) {
      ...
      addAudit({
        ...
        type: "تعديل مشروع",
        description: `تعديل مشروع ${form.nameAr}`,
        page: `projects/${initial.id}`,
        level: "info",
      });
      ...
      addAudit({
        user: currentUser.name,
        type: "إنشاء مشروع",
        description: `إضافة مشروع ${form.nameAr}`,
        page: `projects/${id}`,
        level: "info",
      });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent dir="rtl" className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle className="text-end">{initial ? "تعديل مشروع" : "إضافة مشروع"}</DialogTitle>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <Input
              value={form.facilityAr}
              onChange={(e) => setForm({ ...form, facilityAr: e.target.value })}
            />
          </div>
          <div>
            <Label>القطاع</Label>
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(v) => setForm({ ...form, classification: v as Classification })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {(Object.keys(classificationLabel) as Classification[]).map((s) => (
                  <SelectItem key={s} value={s}>
                    {classificationLabel[s].ar}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
